...
def train():
    ...
    val_dataset = VOCDataset(root_dir="dataset/VOCdevkit", year="2012", mode="val")
    ...
    writer = SummaryWriter(
        f"runs/yolov1_{datetime.now().strftime('%Y-%m-%d_%H_%M_%S')}"
    )
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        if epoch % 1 == 0:
            ...
            print(
                f"Epoch [{epoch+1}/{config.NUM_EPOCHS}], Average Val Loss: {val_loss:.4f}"
            )
        ...
        print(
            f"Epoch [{epoch+1}/{config.NUM_EPOCHS}], Average Train Loss: {avg_loss:.4f}"
        )
