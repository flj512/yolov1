...
from utils.utils import add_padding, convert_cellboxes, non_max_suppression
...
class YOLOPredictor:
    ...
    def draw_boxes(self, img, boxes):
        ...
        for box in boxes:
            ...
            cv2.rectangle(
                img, (x1, y1 - label_h - baseline), (x1 + label_w, y1), color, -1
            )

            # Draw label text
            cv2.putText(
                img, label, (x1, y1 - baseline), font, font_scale, (0, 0, 0), thickness
            )
...
def main():
    ...
    predictor = YOLOPredictor(
        checkpoint_path=MODEL_PATH, conf_threshold=0.6, nms_threshold=0.4
    )
