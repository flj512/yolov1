...
def compare_loss(predict, target, result):
    ...
    assert abs(loss.item() - result) < 1.0e-5, f"Expected loss: {result}, Got: {loss.item()}"
...
def test_loss_1():
    ...
    predict[0,3,5,0:12] = torch.tensor([0.45,0.43,0.2,0.3,0.77,0.55,0.6,0.1,0.1,1.0,0.33,0.26])
    predict[0,6,6,0:11] = torch.tensor([0.5,0.5,0.3,0.4,0.71,0.5,0.5,0.3,0.4,0.63,1.0])
    predict[0,4,5,25] = 1.0
    predict[0,5,5,4] = 0.49
    predict[0,6,0,9] = 0.38

    l01 = LAMBDA_COORD * (wh_error(0.2,0.3)+wh_error(0.3,0.4)+xy_error(0.5,0.45)+xy_error(0.5,0.43))
    l02 = (1.0-0.77)**2
    ...
    predict[1,2,4,0:12] = torch.tensor([0.45,0.43,0.2,0.3,0.68,0.55,0.6,0.1,0.1,1.0,0.33,0.29])
    target[1,2,4,24] = 0.34
    predict[1,2,4,29] = 0.28
    predict[1,6,6,0:11] = torch.tensor([0.5,0.5,0.3,0.4,0.71,0.5,0.5,0.3,0.4,0.63,1.0])
    ...
    compare_loss(predict, target, (l01 + l02 + l03 + l04 +l11 + l12 + l13 + l14)/batch_size)
